...
Start with the [plugin quickstart](quickstart.md) if you have not built a Paseo plugin before.

A provider plugin connects a coding agent to Paseo without adding it to Paseo core. [Publish the plugin](publishing.md) on npm or in a Git repository so users can
...
configuration. See [settings](reference-theme.md#persisted-values) for the `read()` and `subscribe()`
contract.
...
Follow [Publish a plugin](publishing.md) to distribute the provider through npm or Git.
Keep agent compatibility and releases in the plugin project.

See the [plugin reference](reference-project.md#providers) for the exact runtime and SVG
rules.
