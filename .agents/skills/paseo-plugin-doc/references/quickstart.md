...
[surfaces and sidebar items](reference-ui.md#surfaces-and-sidebar-items),
[workspace panels](reference-panels.md#workspace-panels),
[Command Center items](reference-commands.md#command-center-items),
[slash commands](reference-commands.md#slash-commands),
[composer pills](reference-commands.md#composer-pills),
[timeline items](reference-timeline.md#timeline-items),
[themes](reference-theme.md#contribute-a-theme),
[attachment sources](reference-sdk-rpc.md#add-a-composer-attachment-source), and
[daemon-side RPCs](reference-sdk-rpc.md#add-plugin-specific-backend-behavior). It can also
[connect a coding agent as a provider](providers.md). Client
...
[Cross-platform rules](reference-project.md#cross-platform-rules) before writing UI.
...
[Plugin sources](reference-ops.md#plugin-sources) for the accepted syntax and resolution
...
phones. See [Theme and layout](reference-theme.md#theme-and-layout) for the token list.
...
See [plugin sources](reference-ops.md#plugin-sources) for identifier syntax. To share your own plugin, follow
...
Git content once; omitting it installs the default HEAD. See the [source reference](reference-ops.md#plugin-sources) for exact syntax.
...
See the [CLI reference](reference-ops.md#cli-reference) for explicit targets and bulk updates.
...
[Debug backend output](reference-sdk-rpc.md#debug-backend-output) for retention and what not to
log.

## Next

- [Publish a plugin](publishing.md): share through npm, a private registry, or Git.
- [Build a provider plugin](providers.md): connect an agent directly or through ACP,
  render provider-owned timeline items, test it, and publish it.
- [Plugin reference](reference.md): every contribution type, its fields, the runtime
