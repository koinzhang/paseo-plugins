...
- 左侧栏
  ...
  - 「Set up worktree scripts」提示上方：`SidebarCallout` 根节点的 `borderTop`（`components/sidebar-callout.tsx`），按 `[data-testid^="worktree-setup-callout-"][role="alert"]` 匹配，避免误伤同前缀的描边按钮
