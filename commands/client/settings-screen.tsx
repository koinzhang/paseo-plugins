...
import { COMMAND_SECTIONS, COMMANDS, type CommandSection } from "./commands.ts";
...
function CommandSwitches({ settings }: { settings: ReadySettings }) {
  ...
  const lastName = groups.at(-1)?.at(-1)?.name;
  return (
    <>
      {groups.map((group) => (
        <SettingsSection key={group[0].section} title={SECTION_TITLES[group[0].section]}>
          <SettingsCard>
            {group.map((command) => (
              <SettingsSwitch
                key={command.name}
                ...
                error={command.name === lastName ? settings.saveError : null}
              />
            ))}
          </SettingsCard>
        </SettingsSection>
