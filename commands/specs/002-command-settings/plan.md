...
`client/settings-screen.tsx`：按 `CommandSpec.section` 分成多块 `SettingsSection` + `SettingsCard`，标题为 Runtime、Session、Prompt，不加 section 说明，间距由 `SettingsSection` 自己负责，同 Mono。顺序为 runtime（model、effort、profile、mode、feature）、session（rename、cancel）、prompt（resend）。每个命令一个 `SettingsSwitch`（label `/name argumentHint`，hint 为 `details`）。加载失败 / invalid 显示 Reload / Reset，同 Mono。
