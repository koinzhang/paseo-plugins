...
Previously scanned Provider/Project combinations show cached entries immediately when revisited and refresh in the background. A failed refresh keeps the last result visible with a retry action. This cache lives in the current app/plugin runtime and clears when the plugin reloads or the app restarts; scans are not persisted to disk.
