...
- Global Activity adds a rolling 168-hour timeline below the Agents histogram: a diverging filled line chart with messages above the time axis and agents below it. The viewport shows 24 hours at a time and starts at the current hour; drag the horizontal scrollbar to reach the rest of the week. Empty hours keep their slot, and hover / focus / click reveals the local hour range with messages, agents, skill calls, and MCP calls; the window is independent of range chips.
