...
function en({ num }: Formatters) {
  ...
  return {
    ...
    units: {
      ...
      messages: (value: number) => count(value, "message", "messages"),
    ...
    global: {
      ranges: { all: "All time", today: "Today", "7d": "7 days", "30d": "30 days" },
      allProviders: "All",
      heatmapModes: { daily: "Daily", weekly: "Weekly", cumulative: "Cumulative" },
      ...
      emptyHint: "Agents, messages, skill and MCP calls will show up here",
    },
    heatmap: {
      title: "Activity",
    ...
    creations: {
      title: "Agents",
      empty: (days: number) => `No agents created in the last ${num(days)} days`,
    },
    timeline: {
      title: "Timeline",
    ...
    workspace: {
      ...
      options: {
        sort: { updated: "Updated", created: "Created", name: "Name", messages: "Messages", status: "Status" },
        group: { none: "None", provider: "Provider", status: "Status" },
        show: { provider: "Provider", calls: "Calls", messages: "Messages", updated: "Updated", prompt: "Prompt" },
...
function zhCN({ num }: Formatters): Messages {
  return {
    ...
    units: {
      ...
      messages: (value: number) => `${num(value)} 条消息`,
    ...
    insights: {
      ...
      skillCalls: "Skill 调用",
      mcpCalls: "MCP 调用",
      messagesPerAgent: "每个 Agent 的消息",
      codingVsChat: "编码 vs 对话",
      codingShare: (pct: number) => `${num(pct)}% 编码`,
    },
    global: {
      ranges: { all: "全部", today: "今天", "7d": "7 天", "30d": "30 天" },
      allProviders: "全部",
      heatmapModes: { daily: "每日", weekly: "每周", cumulative: "累计" },
      ...
      emptyTitle: "还没有使用记录",
      emptyHint: "Agent、消息、Skill 和 MCP 调用会显示在这里",
    },
    heatmap: {
      title: "Activity",
    ...
    creations: {
      title: "Agent",
      empty: (days: number) => `最近 ${num(days)} 天没有创建 Agent`,
    },
    timeline: {
      title: "时间线",
      legendMessages: "▲ 消息",
    ...
    workspace: {
      ...
      options: {
        sort: { updated: "更新时间", created: "创建时间", name: "名称", messages: "消息", status: "状态" },
        group: { none: "无", provider: "Provider", status: "状态" },
        show: { provider: "Provider", calls: "调用", messages: "消息", updated: "更新时间", prompt: "提示词" },
