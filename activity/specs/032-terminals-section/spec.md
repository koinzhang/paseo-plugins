...
| G2 | 行显示 `name` + `cwd`（homeDir 前缀折叠为 `~`）；点击展开该终端**最近输出**（`capture({ stripAnsi: true })`，最新 8 行），再点收起 |
...
- cwd 显示：`collapseHomePath(cwd, homeDir)`（`shared/home-path.ts`，macOS 大小写不敏感）；homeDir 由新 RPC `usage.host-info`（插件 server `os.homedir()`）提供，客户端 `staleTime: Infinity`
- 列表顺序 = host 返回顺序（创建顺序）
