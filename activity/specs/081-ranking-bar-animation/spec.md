...
- **Ranking Show more / Show less**: the extra rows expand and collapse with a height + opacity transition (250 ms), and the chevron rotates between down and up. On collapse, the rows stay rendered until the animation ends.
- **Ranking rows added / removed by a metric switch**: a newly nonzero position opens with the same height + opacity motion as Show more; a position that drops out closes the same way while showing its last row. This does not apply to first paint (bars grow instead) or to rows revealed by Show more (the disclosure animates them).
...
- **Activity Calendar**: animating ~370 cell colors individually is too costly, so the colored cells of each week column fade in left to right on first display. Switching metric, range, or heatmap mode replays a shorter sweep starting at 30% opacity (the new colors apply immediately). Empty (level 0) cells never animate and must not flash. Background data polling does not replay it.
...
- **Metric switch label**: after pressing › the new metric name slides in from the right; after ‹ it slides in from the left (12px, fading in, 200 ms).
