...
- [x] T3 面板可见时隐藏按钮：`client/panel-visibility.ts` + 面板根 `onLayout` + header `update({ visible })` — 验证：`panel-visibility.test.ts`；typecheck；`npm test` 239 pass；reload 后 running
- [ ] T4 页面验收（spec §4）：按钮出现、点击打开面板后隐藏、切 tab / 关闭 / 收起 Explorer 后重现、新建 / 归档 workspace、切换语言、reload 无重复 — 待真机
