...
4. **`PluginButtonContentProps` / Icon props 类型不含 `navigation`**（`@getpaseo/plugin` 0.8.0）。`openAgent` 仅出现在 Surface / Panel 的 `PluginNavigableHostProps`。因此 pill 内不能类型安全地读 `props.navigation`。
...
| openAgent | 仅 bridge（面板 / Global 注册）；无 `openPanel` 回退（误开 Activity） |
...
- [ ] 运行时 props 是否额外带了未类型化的 `navigation`（可选探测，不依赖）
