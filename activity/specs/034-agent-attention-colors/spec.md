...
| ID | 目标 |
|---|---|
| G1 | `attentionReason = "finished"`（或 `requiresAttention` 且回合完成）：行首机器人图标用 `statusSuccess` 色 |
| G2 | `attentionReason = "error"` 或 `status = "error"`：机器人图标用 `statusDanger` 色 |
| G3 | `permission`：图标用 `statusWarning` 色（与 033 徽标一致） |
...
- 颜色：finished → `theme.colors.statusSuccess`；permission → `theme.colors.statusWarning`；error → `theme.colors.statusDanger`；无 attention → `foregroundMuted`
- 可访问性：行 label 追加 `failed` / `turn finished, unread`（permission 由 033 的计数描述覆盖）

## 5. 验收

- [ ] finished 的 agent 图标为 statusSuccess 色，打开会话后 15s 内恢复 muted（待真机目视确认）
- [ ] error 的 agent 图标为 statusDanger 色；archived 行不染色（待真机目视确认）
- [ ] permission 行图标为 statusWarning 色并显示 warning 徽标（待真机目视确认）
