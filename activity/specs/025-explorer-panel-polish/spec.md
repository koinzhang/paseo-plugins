...
| G3 | Agents 区默认按 **创建时间倒序**；标题右侧设置图标展开**悬浮菜单**（absolute；点卡片外关闭）；视觉与交互对齐宿主侧边栏 `SidebarDisplayPreferencesMenu`：一级 `MenuSurface` 常驻，点/悬停行打开**左侧二级 flyout**（overlap 5）；232 宽、8 圆角、`border`、md 阴影、行 inset hover chip；互斥选项选中后关闭整菜单，Show 开关不关闭 |
...
| G7 | **Show**：多选开关，控制 agent 行副文案是否显示 **Provider / Calls / Messages**（默认三项全开）；无勾选项时不渲染副文案行；行尾数字仍为 `callCount + messageCount`（活动量） |
...
| G10 | **Status**（筛选，单选）：Running（`agents.list` 的 `status === "running"`）/ Active（默认，`archivedAt == null`）/ Archived（`archivedAt != null`）；归档行不提供打开会话链接 |
...
- 行尾数字 = `callCount + messageCount`（与列表活动量一致，不受 Show 开关影响）

## 4. 验收

- [ ] 无 Activity 页标题；分支名与 chips 同字体、同一行对齐；KPI 更紧凑且仅 Shell / File reads / File writes / Messages（实现完成，待真机视觉验收）
- [ ] Status 默认 Active；切到 Running 只列运行中；切到 Archived 列归档 agent；归档行不可点开（实现完成，待真机验收）
- [ ] Show 可独立开关 Provider / Calls / Messages；关净后无副文案（实现完成，待真机验收）
- [ ] 默认显示 Skills；点标题右侧图标切到 MCP（实现完成，待真机验收）
- [ ] 设置菜单：点图标展开一级悬浮卡片；点/悬停 Sort / Group / Show / Status 打开**二级 flyout**；互斥选项选中后关闭整菜单；Show 开关保持打开；点卡片外 / 再点图标收起（实现完成，待真机验收）
